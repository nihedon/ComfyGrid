...
>
  {#each workflowState.currentFolderItems as item (item.path)}
    ...
    <!-- svelte-ignore a11y_click_events_have_key_events -->
    <div
      class="card position-relative shadow-sm workflow-card cursor-pointer border position-relative"
    ...
    >
      ...
      <div class="card-body py-1 px-2 d-flex flex-column justify-content-between">
        {#if isRenaming}
          ...
          />
        {:else}
          <span class="card-title fw-semibold text-truncate mb-0 fs-6" title={item.name}>
            {item.name}
          </span>
        {/if}
